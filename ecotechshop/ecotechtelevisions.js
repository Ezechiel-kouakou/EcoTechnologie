const EcoTech_API_URL = "/ecotechnologie/ecotech_api/ecotechtelevisions.php";
const EcotechCART_URL = "/ecotechnologie/ecotech_api/ecotech_panier.php";
const CATEGORIE_NOM = "televisions";
const DETAILS_PAGE = "tv_details.html";
...
/**
 * Navigation et Session (Version Alignée)
 */
function gererAffichageNav() {
    ...
    // Nettoyage des anciens boutons dynamiques dans le menu burger
    ...
    if (userData) {
        ...
        // NAVBAR : Affichage de l'icône profil (Lien corrigé)
        ...
        // MENU BURGER : Option déconnexion
        if (navMenu) {
        ...
        // NAVBAR : Icône par défaut (Lien corrigé pour cohérence)
        authContainer.innerHTML = `
        <div class="user-info">
            <div class="user-name">
                <img 
        ...
        // MENU BURGER : Connexion et Inscription
        if (navMenu) {
            navMenu.insertAdjacentHTML("beforeend", `
                <li class="dynamic-auth">
    ...
    if (token) {
        try {
            ...
            if (result.status === 'success') {
                ...
                localStorage.removeItem("ecotech_token");
                setTimeout(() => window.location.href = "../ecotechnologie/ecotechshop/auth_ecotech.html", 2000);
            } else {
...
/**
 * Fetch et Affichage des Télévisions
 */
async function fecthProducts() {
    ...
    try {
        ...
        if (data.status !== 'success' || !data.products) {
            productsList.innerHTML = '<p style="text-align:center; grid-column: 1/-1;">Aucun téléviseur disponible pour le moment.</p>';
            return;
        }
        ...
        showToast("Impossible de charger le catalogue TV", "error");
    }
}

// --- ÉCOUTEURS D'ÉVÉNEMENTS ---
document.addEventListener("DOMContentLoaded", () => {
    injectProSessionUrl(); // Initialisation de l'URL sécurisée
    gererAffichageNav();
    fecthProducts();
    updateCartBadge(); 
});
