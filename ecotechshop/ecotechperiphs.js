const EcoTech_API_URL = "/ecotechnologie/ecotech_api/ecotechperiphs.php";
const EcotechCART_URL = "/ecotechnologie/ecotech_api/ecotech_panier.php";
const CATEGORIE_NOM = "ecotechperiphs";
const DETAILS_PAGE = "periph_details.html";

// --- SYSTEME DE SESSION PRO ---
/**
 * Génère un identifiant de session complexe
 */
...
/**
 * Injecte les paramètres de session dans l'URL (Look professionnel)
 */
function injectProSessionUrl() {
    ...
    if (!urlParams.has('session_id')) {
        ...
        urlParams.set('view', 'peripherals_catalog');
...
async function fecthProducts() {
    ...
    try {
        ...
        if (data.status !== 'success' || !data.products) {
            productsList.innerHTML = '<p style="text-align:center; grid-column: 1/-1;">Aucun périphérique disponible.</p>';
            return;
        }
...
document.addEventListener("DOMContentLoaded", () => {
    injectProSessionUrl(); // Activation du marquage de session dans l'URL
    gererAffichageNav();
    fecthProducts();
    updateCartBadge(); 
});
