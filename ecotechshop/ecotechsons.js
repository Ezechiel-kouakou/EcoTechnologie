const EcoTech_API_URL = "/ecotechnologie/ecotech_api/ecotechsons.php";
const EcotechCART_URL = "/ecotechnologie/ecotech_api/ecotech_panier.php";
const CATEGORIE_NOM = "sons";
const DETAILS_PAGE = "sons_details.html";
...
function injectProSessionUrl() {
    ...
    if (!urlParams.has('session_id')) {
        ...
        urlParams.set('view', 'audio_catalog');
...
function gererAffichageNav() {
    ...
    if (userData) {
        ...
        // NAVBAR (affichage icône utilisateur)
        ...
        // MENU BURGER (Ajout de la déconnexion)
        if (navMenu) {
        ...
        // NAVBAR (affichage icône par défaut)
        authContainer.innerHTML = `
        <div class="user-info">
            <div class="user-name">
                <img 
        ...
        // MENU BURGER (Connexion / Inscription)
        if (navMenu) {
            navMenu.insertAdjacentHTML("beforeend", `
                <li class="dynamic-auth">
...
/**
 * Fetch et Affichage des produits
 */
async function fecthProducts() {
    ...
    try {
        ...
        if (data.status !== 'success' || !data.products) {
            productsList.innerHTML = '<p style="text-align:center; grid-column: 1/-1;">Aucun produit audio disponible pour le moment.</p>';
            return;
        }
        ...
        showToast("Impossible de charger le catalogue audio", "error");
    }
}

// --- ÉCOUTEURS D'ÉVÉNEMENTS ---
document.addEventListener("DOMContentLoaded", () => {
    injectProSessionUrl(); // Activation de l'URL Pro au chargement
    gererAffichageNav();
    fecthProducts();
    updateCartBadge(); 
});
